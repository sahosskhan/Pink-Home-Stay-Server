...
// middleware
const corsOptions = {
  origin: ['http://localhost:5173', 'http://localhost:5174', 'https://pink-home-stay.web.app','https://pink-home-stay.firebaseapp.com'],
  credentials: true,
...
async function run() {
  try {
    const usersCollection = client.db('PinkHomeStayDB').collection('users')
    const roomsCollection = client.db('PinkHomeStayDB').collection('rooms')
    const bookingsCollection = client.db('PinkHomeStayDB').collection('bookings')
   ...
   app.put('/users/update-data/:email', async (req, res) => {
    const email = req.params.email
    const user = req.body
    const query = { email: email }
    const options = { upsert: true }
...
app.get('/', (req, res) => {
  res.send('Hello from Pink Stay Home Server..')
})

app.listen(port, () => {
  console.log(`Pink Stay Home is running on port ${port}`)
})
